...
(function() {
  ...
  RL.PushReport = function(htmlId) {
    ...
    YAHOO
        .extend(
            ...
            {
              /**
               * Called by the ConsolePanelHandler when this panel shall be
               * loaded
               * 
               * @method onLoad
               */
              onLoad : function onLoad() {
                parent.widgets.fromDateField = Dom
                    .get(parent.id + "-from-date");
                parent.widgets.fromTimeField = Dom
                    .get(parent.id + "-from-time");
                parent.widgets.toDateField = Dom.get(parent.id + "-to-date");
                parent.widgets.toTimeField = Dom.get(parent.id + "-to-time");
                parent.widgets.publishStatusField = Dom.get(parent.id
                    + "-publishstatus");
                parent.widgets.unpublishStatusField = Dom.get(parent.id
                    + "-unpublishstatus");
                // Buttons
                parent.widgets.pushButton = Alfresco.util.createYUIButton(
                    parent, "report-button", parent.onReportClick);

                // DataSource
                parent.widgets.dataSource = new YAHOO.util.DataSource(
                    Alfresco.constants.PROXY_URI + "vgr/publishreport", {
              ...
              _setupDataTable : function() {
                ...
                var renderCellPushedForPublish = function(cell, record, column,
                    data) {
                  cell.innerHTML = $html(data);

                };

                var renderCellPushedForUnpublish = function(cell, record,
                    column, data) {
                  cell.innerHTML = $html(data);

                };

                var renderCellPublishStatus = function(cell, record, column,
                    data) {
                  cell.innerHTML = $html(data);

                };

                var renderCellUnpublishStatus = function(cell, record, column,
                    data) {
                  cell.innerHTML = $html(data);
                ...
                var generateRequest = function(oState, oSelf) {
                  ...
                  var from = parent.widgets.fromDateField.value + "T"
                      + parent.widgets.fromTimeField.value + ":00";
                  var to = parent.widgets.toDateField.value + "T"
                      + parent.widgets.toTimeField.value + ":00";
                  // Build custom request
                  return "?publishstatus=" + publishStatus
                      + "&unpublishstatus=" + unpublishStatus + "&from=" + from
                      + "&to=" + to;
                };

                parent.widgets.dataTable = new YAHOO.widget.DataTable(parent.id
                    + "-result", columnDefinitions, parent.widgets.dataSource,
                    {
                      sortedBy : {
                        key : "title",
                        dir : "asc"
  ...
  YAHOO.extend(RL.PushReport, Alfresco.ConsoleTool, {
    ...
    doUpdate : function PR_Update() {
      ...
      this.widgets.dataTable.set("MSG_EMPTY", Alfresco.util.message(
          "message.empty", "RL.PushReport"));

      // empty results table
      this.widgets.dataTable.deleteRows(0, this.widgets.dataTable
          .getRecordSet().getLength());
      ...
      var fnShowLoadingMessage = function() {
        loadingMessage = Alfresco.util.PopupManager.displayMessage({
          displayTime : 0,
          text : '<span class="wait">' + $html(this.msg("message.loading"))
              + '</span>',
          noEscape : true
        });
      };
      ...
      var timerShowLoadingMessage = YAHOO.lang.later(2000, this,
      ...
      var successHandler = function(request, response, payload) {
        ...
        this.widgets.dataTable.onDataReturnInitializeTable.call(
            this.widgets.dataTable, request, response, payload);
      };
      ...
      var from = this.widgets.fromDateField.value + "T"
          + this.widgets.fromTimeField.value + ":00";
      var to = this.widgets.toDateField.value + "T"
          + this.widgets.toTimeField.value + ":00";
      this.widgets.dataSource.sendRequest("?publishstatus=" + publishStatus
          + "&unpublishstatus=" + unpublishStatus + "&from=" + from + "&to="
          + to, {
