...
//var template = '<div id="${id}" class="notifcation" style="margin-top: 10px; padding: 15px; margin-left: 10px; margin-right: 10px;"> <a class="theme-color-2 invite-yes" href="#" style="padding-left: 20px; font-weight: bold;">Ja tack! </a><a href="#" class="theme-color-2 invite-no" style="padding-left: 10px;">Nej inte nu. </a></div>';
...
var SystemNotifications = function() {
    ...
    Alfresco.util.Ajax.jsonGet({
        url: '/share/proxy/alfresco/vgr/notifications',
        successCallback: { 
            fn:  function (res) {
...
var Invitations = function() {
    ...
    //Do an ajax request to check for messagexs
    Alfresco.util.Ajax.jsonGet({
        url: '/share/proxy/alfresco/vgr/invite/invitations',
        successCallback: { 
            fn:  function (res) {
                    if (res.json) {
                        //If the server is really fast we get a response before the DOM is ready, at least its possible theoretically
                        Event.onDOMReady(function(){
                            me._callback(res.json.invites);
                        });
                    }
                }
           }
...
Invitations.prototype._callback = function(invites) {
    for (var i=0; i<invites.length; i++) {
        var inv = invites[i];
        
        //'Vill du bli medlem? <a class="theme-color-2 invite-yes" href="#" style="padding-left: 20px; font-weight: bold;">Ja tack! </a><a href="#" class="theme-color-2 invite-no" style="padding-left: 10px;">Nej inte nu. </a>'
        Bubbling.fire('notifications.notify',{
                                        id: inv.inviteId,    
                                        title:  'Du har blivit inbjuden till platsen <em>'+inv.site+'</em>',
                                        text:   'Vill du bli medlem? <a id="'+inv.inviteId+'-invite-yes" class="invite-yes" href="#">Ja tack! </a><a id="'+inv.inviteId+'-invite-no" href="#" class="invite-no">Nej inte nu. </a>'
                                    });
                              
                                
        //hook up events
        (function(id){ //closure to capture id
            //accept
            Event.on(inv.inviteId+'-invite-yes','click',function(e){
                Event.stopPropagation(e);
                this._send_req(id,
                               Alfresco.constants.PROXY_URI + "vgr/invite/accept",
                               "Du har tackat ja till inbjudan",
                               "Ett fel har uppstått, inbjudan ligger kvar",
                               "siteAdded"
                );
            },null,this);
            //cancel
            Event.on(inv.inviteId+'-invite-no' ,'click',function(e){
                Event.stopPropagation(e);
                this._send_req(id,
                               Alfresco.constants.PROXY_URI + "vgr/invite/reject",
                               "Du har tackat nej till inbjudan",
                               "Ett fel har uppstått, inbjudan ligger kvar"
                );
            },null,this);
        }).call(this,inv.inviteId);
    }
};

Invitations.prototype._send_req = function(id,url,success,failure,event) {
