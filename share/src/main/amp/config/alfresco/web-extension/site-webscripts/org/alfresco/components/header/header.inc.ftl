<#-- @overridden projects/slingshot/config/alfresco/site-webscripts/org/alfresco/components/header/header.inc.ftl -->
...
<div id="${id}-${prefix}menu_${item.generatedId}" class="yuimenu">
      ...
      YAHOO.util.Event.on("${id}-${prefix}menu_${item.generatedId}", "mouseover", function (e) {
         ...
         iframe.width=bdDiv.offsetWidth;
         iframe.height=bdDiv.offsetHeight+3;    
      });
      
      YAHOO.util.Event.on("${id}-${prefix}_${item.generatedId}-button", "mouseover", function (e) {
         var iframe = YAHOO.util.Dom.get("${id}-${prefix}menu_${item.generatedId}-iframe");
         var bdDiv = YAHOO.util.Dom.get("${id}-${prefix}menu_${item.generatedId}-bd"); 
         iframe.width=bdDiv.offsetWidth;
         iframe.height=bdDiv.offsetHeight+3;    
      ...
      YAHOO.util.Event.on("${id}-${prefix}_${item.generatedId}", "mouseover", function (e) {
         var iframe = YAHOO.util.Dom.get("${id}-${prefix}menu_${item.generatedId}-iframe");
         var bdDiv = YAHOO.util.Dom.get("${id}-${prefix}menu_${item.generatedId}-bd"); 
         iframe.width=bdDiv.offsetWidth;
         iframe.height=bdDiv.offsetHeight+3;    
   ...
   <div style="position:absolute;z-index:20;">
      <iframe visible="true" id="${id}-${prefix}menu_${item.generatedId}-iframe" height="500" width="100" frameborder="0" scrolling="auto" >
      </iframe>
   </div>
   
   <div id="${id}-${prefix}menu_${item.generatedId}-bd" class="bd" style="position:absolute;z-index:30;">
