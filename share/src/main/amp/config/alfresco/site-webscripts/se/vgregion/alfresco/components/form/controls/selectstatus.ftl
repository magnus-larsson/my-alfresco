   ...
   <#else>
      ...
      <#if field.control.params.options?? && field.control.params.options != "">
         <select id="${fieldHtmlId}" name="${field.name}" tabindex="0"
               ...
               <option id="" value="">${msg("from.control.selectstatus.empty-title")}</option>
               <#list field.control.params.options?split(optionSeparator) as nameValue>
                  <#if nameValue?index_of(labelSeparator) == -1>
                     <option id="${nameValue?html}" value="${nameValue?html}"<#if nameValue == fieldValue?string || (fieldValue?is_number && fieldValue?c == nameValue)> selected="selected"</#if>>${nameValue?html}</option>
                  <#else>
                     <#assign choice=nameValue?split(labelSeparator)>
                     <option id="${choice[0]?html}" value="${choice[1]?html}">${msgValue(choice[1])?html}</option>
                  </#if>
               </#list>
         </select>
      ...
      (function() {
         new Alfresco.thirdparty.SelectDocumentStatus('${fieldHtmlId}').setOptions({
