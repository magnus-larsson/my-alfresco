...
(function()
{
    ...
    * @constructor
    */
   Alfresco.dashlet.MyDocuments = function MyDocuments_constructor(htmlId,site,user)
   {
      ...
      //site if dashlet is on site dashboard, otherwise undefined
      this.site = site;
      
      //current user, used for cancel editing buttons
      this.user = user;
      
      //starting filter
      if (site) {
         this.currentFilter = "editingMe";
      } else {
         this.currentFilter = "favourites";
      }
   ...
   YAHOO.extend(Alfresco.dashlet.MyDocuments, Alfresco.component.Base,
   {
      ...
      onReady: function MyDocuments_onReady()
      {
         ...
         if (this.site) {
            var uriDocList = Alfresco.constants.PROXY_URI + "slingshot/doclib/doclist/{type}/site/"+this.site+"/documentLibrary?max=50&filter=";
         } else {
          ...
          * @param oData {object|string}
          */
         var checkinMsg    = this.msg('label.checkin');
         var cancelMsg     = this.msg('label.cancel.editing');
         var titleTemplate = this.msg('title.cancel.editing');
         var renderCellAction = function MD_renderCellAction(elCell, oRecord, oColumn)
         {
            ...
            if (data.custom && data.custom.isWorkingCopy && data.custom.isWorkingCopy === true &&
                data.permissions && data.permissions.userAccess) {
                           
               Dom.setStyle(elCell, "width", oColumn.width + "px");
               Dom.setStyle(elCell.parentNode, "width", oColumn.width + "px");
               var title = titleTemplate.replace('{0}',data.lockedBy);
               
               //if locked by use offer check in
               if (data.lockedByUser === me.user && data.permissions.userAccess.edit) {
                  elCell.innerHTML = '<button>'+checkinMsg+'</button>';
                  
                  //make a checkin button
                   Alfresco.thirdparty.createCheckInButton(Dom.getFirstChild(elCell),
                                                           data.nodeRef,
                                                           data.displayName,
                                                           me._onCheckIn,
                                                           me);
                  ...
                  elCell.innerHTML = '<button title="'+title+'">'+cancelMsg+'</button>';
                  
                  //make a checkin button
                  Alfresco.thirdparty.createCancelEditingButton(Dom.getFirstChild(elCell),
                                                           data.nodeRef,
                                                           function() { this._filterChange(this.currentFilter); },
                                                           me);
         ...
         if (this.site) {
            var columnDefinitions = [
               ...
               { key: "action",      label: "Action",      sortable: false, formatter: renderCellAction, width: 120 }
            ];   
         } else {   
            //User dashboard
            var columnDefinitions = [
      ...
      {
         this._filterChange(p_obj.newValue.get("value"));
      },
      
      _filterChange: function MD_filterChange(filter)
      {
      ...
      /**
       * Show file upload dialog for checkin
       *
       * @method onCheckin
       * @param data {object} DataTable record representing file to be actioned
       */
      _onCheckIn: function (nodeRef,filename,data) {
         //reload our current filter 
         this._filterChange(this.currentFilter);
