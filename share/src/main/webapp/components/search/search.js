...
(function()
{
   ...
   YAHOO.extend(Alfresco.Search, Alfresco.component.Base,
   {
         ...
         renderCellDescription = function Search_renderCellDescription(elCell, oRecord, oColumn, oData)
         {
            ...
            if (type === 'document') {
               if (!site) {
                  ...
                  var noSourceAndPublished = permissions['no-source-and-published'] ? me.msg('label.yes') : me.msg('label.no');
                  var unknownSourceAndPublished = permissions['unknown-source-and-published'] ? me.msg('label.yes') : me.msg('label.no');
                  ...
                  if (origin === 'Alfresco') {
                     if (permissions['no-source-and-published'] === true) {
                        desc += '<div class="details">' + me.msg("message.no-source-and-published") + ': <a href="javascript:void(0);" id="' + linkId + '">' + noSourceAndPublished + '</a></div>';
                        
                        Event.onAvailable(linkId, function(record) {
                           var args = { record: record, scope: me };
                           Event.addListener(linkId, "click", me.onUnpublish, args);
                        }, oRecord); 
                     } else {
                        desc += '<div class="details">' + me.msg("message.no-source-and-published") + ': ' + noSourceAndPublished + '</div>';
                     }
                  } else {
                     if (permissions['unknown-source-and-published'] === true) {
                        desc += '<div class="details">' + me.msg("message.unknown-source-and-published", origin) + ': <a href="javascript:void(0);" id="' + linkId + '">' + unknownSourceAndPublished + '</a></div>';
                        ...
                        desc += '<div class="details">' + me.msg("message.unknown-source-and-published", origin) + ': ' + unknownSourceAndPublished + '</div>';
                     }
                  }
               }
            }
      ...
      onUnpublish: function(e, args) {
         var record = args.record;
         var me = args.scope;
         
         Alfresco.util.PopupManager.displayPrompt({
            title: me.msg("message.confirm.unpublish.title"),
            text: me.msg("message.confirm.unpublish"),
            buttons: [{
               text: me.msg("button.unpublish"),
               handler: function () {
                  this.destroy();
                  me._unpublishOrphanDocument(record, me);
               }
            },
            {
      ...
      _unpublishOrphanDocument: function(record, scope) {
         var me = scope;
            ...
            successCallback : {
               fn : function(res) {
                  Alfresco.util.PopupManager.displayMessage({
                     text : me.msg('unpublish.success'),
                     displayTime : 3
                  });
                  
                  var anchor = Dom.get(record.getData().nodeRef + "-unpublish");
                  
                  anchor.outerHTML = me.msg('label.no');
