// @overridden projects/slingshot/source/web/components/site/customise-pages.js
...
(function()
{
   ...
   Alfresco.CustomisePages.prototype =
   {
      ...
      onComponentsLoaded: function CP_onComponentsLoaded()
      {
         ...
         this.widgets.pagesUl = Dom.get(this.id + "-currentPages-ul");
         ...
         //default page menu
         this.widgets.defaultpage = new YAHOO.widget.Button(this.id + "-default-page",
         {
            type: "split",
            menu: this.id + "-default-page-select",
            lazyloadmenu: false
         });
         
         var me = this;
         this.widgets.defaultpage.getMenu().subscribe("click", function (p_sType, p_aArgs)
         {
            var menuItem = p_aArgs[1];
            if (menuItem)
            {
               me.widgets.defaultpage.set("label", menuItem.cfg.getProperty("text"));
               me.options.defaultPage = menuItem.value;
            }
         });
         

         //add d'n'd functionality for reordering
         this.widgets.dnd = new Alfresco.util.DragAndDrop(
         {
            draggables: [
               {
                  container: this.widgets.pagesUl,
                  groups: [Alfresco.util.DragAndDrop.GROUP_MOVE],
                  cssClass: "customise-pages-page"
               }
            ],
            targets: [
               {
      ...
      onSelectButtonClick: function CP_onSelectButtonClick(event, button)
      {
         ...
         //add the selected page to the default page menu
         var me = this;
         var addItem = function() { 
            me.widgets.defaultpage.getMenu().addItem({ 
                  label: me.options.pages[selectedPageId]?me.options.pages[selectedPageId]:selectedPageId,
      ...
      onRemoveButtonClick: function CP_onRemoveButtonClick(event, obj)
      {
         ...
         //remove it from the default page menu
         var menu = obj.thisComponent.widgets.defaultpage.getMenu();
         var items = menu.getItems();
         for (var i=0;i<items.length; i++) {
            if (obj.selectedPageId == items[i].value) {
               menu.removeItem(items[i]);
               break;
            }
         }

         //set dashboard as default if the removed page was set as default page
         if (obj.thisComponent.options.defaultPage == obj.selectedPageId) {
            obj.thisComponent.options.defaultPage = "dashboard";
            obj.thisComponent.widgets.defaultpage.set("label",
                        Alfresco.util.message("dashboard", obj.thisComponent.name)
            );
         }
      ...
      onSaveButtonClick: function CD_onSaveButtonClick(event)
      {
         ...
         for (var i = 0; i < children.length; i++)
         {
            ...
            var defaultPage = this.options.defaultPage == pageId;
