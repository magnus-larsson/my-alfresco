// @overridden projects/slingshot/source/web/components/document-details/path.js
  ...
  YAHOO.extend(Alfresco.component.Path, Alfresco.component.Base,
    {
      ...
      onFolderDetailsAvailable: function Path_onFolderDetailsAvailable(layer, args)
      {
        ...
        {
          Dom.get(this.id + "-iconType").innerHTML = YAHOO.lang.substitute('<img src="{iconContext}{icon}-48.png" width="48" height="48" />{statusIcon}<span class="type">{type}</span>',
            {
              ...
              statusIcon: this._constructStatusIcon(folderData.status)
            });
        }
      },
      ...
      _constructStatusIcon: function(dataStatus) {
        ...
        for (var i = 0, j = statuses.length; i < j; i++) {
          ...
          if (meta && this.msg(i18nMeta) !== i18nMeta) {
            i18n = i18nMeta;
          }

          tip = Alfresco.util.message(i18n, this.name, meta.split("|")); // Note: deliberate bypass of scope.msg() function

          desc += '<img class="status-icon" src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/images/' + status + '-indicator-16.png" title="' + tip + '" alt="' + status + '" />';
        }
