// @overridden projects/slingshot/source/web/modules/create-site.js
...
(function()
{
   ...
   YAHOO.extend(Alfresco.module.CreateSite, Alfresco.component.Base,
   {
      ...
      onTemplateLoaded: function CS_onTemplateLoaded(response)
      {
         ...
         //shortname is autofilled from title
         var shortInput = Dom.get(this.id + "-shortName");
         //set tabindex so we don't tab to it
         shortInput.tabindex = 100;
         //disable it
         Dom.addClass(shortInput, "disabled");
         Event.addListener(shortInput,'click',function(){
        	 Dom.removeClass(shortInput,'disabled');
         });
         //update help text with actual url
         var helpSpans = Selector.query('#'+this.id + "-form .help");
         var reHelp = new RegExp('/site/[^/]*/dashboard');
         //update if typing
         var helpSpanUpdater = function(e) {
        	 if (helpSpans.length > 0) {
        		 helpSpans[0].innerHTML = helpSpans[0].innerHTML.replace(reHelp,'/site/'+shortInput.value+'/dashboard');
        	 }
         }
         Event.addListener(shortInput,'keyup',helpSpanUpdater);
         
         //calculate shortname from title
         Event.addListener(this.id + "-title",'keyup',function(e,obj){
        	 if (obj.title.value && Dom.hasClass(shortInput,"disabled")) {
        		 shortInput.value = obj.title.value.replace(obj.re_space,'-')
        		                                   .replace(obj.re_aring_auml,'a')
        		                                   .replace(obj.re_ouml,'o')
        		                                   .replace(obj.re_clean,'').toLowerCase();
        		 createSiteForm.updateSubmitElements();
        		 helpSpanUpdater();
        	 }
         },{
            title: Dom.get(this.id + '-title'),
        	re_clean: new RegExp('[^0-9a-zA-Z_-]','g'),
        	re_space: new RegExp('[ ]','g'),
        	re_aring_auml: new RegExp('[åÅäÄ]','g'),
        	re_ouml: new RegExp('[öÖ]','g')
        	});
