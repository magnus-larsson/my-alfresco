<#-- @overridden projects/slingshot/config/alfresco/site-webscripts/org/alfresco/components/documentlibrary/include/documentlist.lib.ftl -->
...
<#list actionSets?keys as key>
   ...
   <#list actionSet as action>
         <div class="${action.id}"><a rel="${action.permission!""}" href="${action.href}" class="${action.type}" title="${msg(action.title)}"><span>${msg(action.label)}</span></a></div>
   </#list>
      </div>
</#list>
   </div>
