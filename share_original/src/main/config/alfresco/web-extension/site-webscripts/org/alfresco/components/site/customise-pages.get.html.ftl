...
<div id="${args.htmlid}" class="customise-pages">
   ...
   var CustomisePages = new Alfresco.CustomisePages("${args.htmlid}").setOptions(
   {
      siteId: "${siteId}",
   ...
   <div id="${args.htmlid}-currentPages-div" class="currentPages">
      ...
      <div>
         <ul id="${args.htmlid}-currentPages-ul">
            ...
            <#list pages as page>
            <li id="${args.htmlid}-currentPage-li-${page.pageId}" class="customise-pages-page" <#if (!page.used)>style="display: none;"</#if>>
      ...
      <div>
         <div class="buttons" id="${args.htmlid}-addPages-div">
            <span class="defaultPage">
               <label>${msg("label.defaultPage")}</label>
               <input id="${args.htmlid}-default-page" type="button" name="defaultPage" value="${msg(defaultPage)}" />
               <select id="${args.htmlid}-default-page-select">
                  <#list pages as page>
                     <#if (page.used)> 
                        <option value="${page.pageId}">${page.title}</option>
                     </#if>
                  </#list>
                     <option value="dashboard">${msg("dashboard")}</option>
               </select>
            </span>
