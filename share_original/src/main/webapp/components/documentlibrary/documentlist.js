   ...
   YAHOO.lang.augmentObject(Alfresco.DocumentList.prototype,
      {
         ...
         fnRenderCellStatus: function DL_fnRenderCellStatus()
         {
            ...
            return function DL_renderCellStatus(elCell, oRecord, oColumn, oData)
            {
               ...
               var published         = record.permissions.userAccess['published'];
               var published_before  = record.permissions.userAccess['published-before'];
               var will_be_published = record.permissions.userAccess['will-be-published'];
               var older_version_published = record.permissions.userAccess['older-version-published'];
               var repo_published    = record.permissions.userAccess['repo-published'];

               if (will_be_published) {
                  desc += '<div class="status"><img src="' + Alfresco.constants.URL_CONTEXT + 'components/documentlibrary/images/published-to-storage-future-16.png" title="Denna version kommer att bli publicerad" alt="Denna version kommer att bli publicerad" /></div>';
               } else  if (published) {
                  desc += '<div class="status"><img src="' + Alfresco.constants.URL_CONTEXT + 'components/documentlibrary/images/published-to-storage-16.png" title="Publicerad" alt="Publicerad" /></div>';
               } else if (older_version_published) {
                  desc += '<div class="status"><img src="' + Alfresco.constants.URL_CONTEXT + 'components/documentlibrary/images/published-to-storage-older-version-16.png" title="Tidigare version är publicerad" alt="Tidigare version är publicerad" /></div>';
               } else if (published_before) {
                  desc += '<div class="status"><img src="' + Alfresco.constants.URL_CONTEXT + 'components/documentlibrary/images/published-to-storage-before-16.png" title="Tidigare version har varit publicerad" alt="Tidigare version har varit publicerad" /></div>';
               }
         ...
         onEventHighlightRow: function DL_onEventHighlightRow(oArgs)
         {
            ...
            if (elActions && elActions.firstChild === null)
            {
               ...
               for (i = 0, ii = actions.length; i < ii; i++)
               {
                   ...
                   */
                  if ((action.className == 'onActionDownload' || action.className == 'onActionCopyTo') && !userAccess.edit && this.isVideo(record.getData("mimetype")) && this.options.containerType == 'vgr:prevent-download') {
                     clone.removeChild(action);
                  }
               }
          ...
          */
         isVideo: function DL_isVideo(mimetype) {
