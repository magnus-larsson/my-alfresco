...
(function()
{
   ...
   */
   ...
   {
      Alfresco.component.DataGrid.superclass.constructor.call(this, "Alfresco.component.DataGrid", htmlId, ["button", "container", "datasource", "datatable", "paginator", "animation", "history", "dragdrop"]);
   ...
   YAHOO.lang.augmentObject(Alfresco.component.DataGrid.prototype,
   {
      ...
      options:
      {
          ...
          * @property moveUrl
          * @type string
          * @default Alfresco.constants.URL_SERVICECONTEXT + "vgr/data-lists/move"
          */
         moveUrl: Alfresco.constants.PROXY_URI + "vgr/data-lists/move"
      },
      ...
      _setupDataTable: function DataGrid__setupDataTable(columns)
      {
         ...
         columnDefinitions.push(
            { key: "actions", label: this.msg("label.column.actions"), sortable: false, formatter: this.fnRenderCellActions(), width: 110 }
         );
      ...
      /**
       * Triggered on cellMousedownEvent from the datatable,
       * starts a drag. 
       */
      onRowDragStart: function DataGrid__onRowDragStart(ev) 
      { 
         var par = this.widgets.dataTable.getTrEl(Event.getTarget(ev)), 
            srcData, 
            srcIndex, 
            tmpIndex = null,
            me = this, 
            ddRow = new YAHOO.util.DDProxy(par.id,this.datalistMeta.itemType); 
         ...
         ddRow.onDragDrop = function (e,target,group) {
            ...
            if (src && trgt) {
               ...
               Alfresco.util.Ajax.request({
		            ...
		            dataObj: { srcs:srcs, target:trgt  }, 
		            successCallback: {fn: function() { 
		               popup.hide();
		               me.onDataItemsDeleted(null,[null,{items:[{ nodeRef: src}]}]);
		            }, scope: me },
		            successMessage: me.msg("message.moving.success"),
		            failureCallback: { fn: popup.hide, scope: popup },
