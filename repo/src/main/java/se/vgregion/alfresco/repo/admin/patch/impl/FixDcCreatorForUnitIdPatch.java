...
public class FixDcCreatorForUnitIdPatch extends AbstractPatch implements InitializingBean {

  private static final Logger LOG = Logger.getLogger(FixDcCreatorForUnitIdPatch.class);

  private static final String MSG_SUCCESS = "vgr.patch.fixDcCreatorForUnitIdPatch.result";
  ...
  private String doApply() {
    ...
    try {
      for (final ResultSetRow document : documents) {
        ...
        final Serializable storedUnits = document.getValue(VgrModel.PROP_CREATOR_FORUNIT);
        ...
        if (unitIds.size() == 0 || units.size() == 0) {
          LOG.warn("The dc.creator.forunit should not be blank for " + document.getNodeRef());

          continue;
        }

        // set the dc.creator.forunit
        nodeService.setProperty(document.getNodeRef(), VgrModel.PROP_CREATOR_FORUNIT, (Serializable) units);

        // set the dc.creator.forunit.id
        nodeService.setProperty(document.getNodeRef(), VgrModel.PROP_CREATOR_FORUNIT_ID, (Serializable) unitIds);

        LOG.debug("Patched vgr:dc.creator.forunit.id for " + document.getNodeRef());
      }
    } finally {
  ...
  private ResultSet queryDocuments() {
    final String query = "TYPE:\"vgr:document\" AND (ISUNSET:\"vgr:dc.creator.forunit.id\" OR ISNULL:\"vgr:dc.creator.forunit.id\")";
