...
public class KivUnitSynchronisationImpl implements InitializingBean,
    ...
    private static final QName LOCK = QName.createQName(NamespaceService.SYSTEM_MODEL_1_0_URI, "KivUnitSynchronization");
    /** Default Lock time to live: 15 minutes */
    private static final long LOCK_TTL = 15*60*1000L;
	...
	public void synchronise() {
		try {
			...
			if (token != null) {
				...
				LOG.info("Job is already running, exiting.");
			}
		} finally {
			String token = this.lockToken.get();
            if (token != null)
            {
                _jobLockService.releaseLock(token, LOCK);
            }
		}
	}
	...
	private void deleteRemovedUnits(List<NodeRef> actualNodes) {
		...
		List<FileInfo> listDeepFolders = _fileFolderService.listDeepFolders(
				getKivTypeNode(), null);
		...
		for (NodeRef folderToDelete : foldersToDelete) {
			if (_nodeService.exists(folderToDelete)) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("Deleting node " + folderToDelete.toString()
							+ " and its children.");
				}
			}
		}
	...
	private void synchroniseUnits(final List<NodeRef> actualNodes,
			final List<KivUnit> units) {
		synchroniseUnits(actualNodes, units, null);
	}

	private void synchroniseUnits(final List<NodeRef> actualNodes,
			final List<KivUnit> units, final KivUnit parentUnit) {
		for (final KivUnit unit : units) {
					...
					.doInTransaction(
							new RetryingTransactionHelper.RetryingTransactionCallback<Object>() {
								...
								public Object execute() throws Throwable {
									synchroniseUnit(actualNodes, unit,
											parentNodeRef);
									return null;
								}
			...
			if (subunits.size() > 0) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("Processing " + subunits.size()
							+ " subunits for " + unit.getDistinguishedName());
				}
	...
	private void synchroniseUnit(final List<NodeRef> actualNodes,
			final KivUnit unit, final NodeRef parentNodeRef) {
	...
	private String getLock() {
		try {
			return _jobLockService.getLock(LOCK, LOCK_TTL);
	...
	public void afterPropertiesSet() throws Exception {
		...
		Assert.notNull(_jobLockService);
	}
