...
function main()
{
   ...
   var roles = {};
   var zones = Array(memberships.length);
   for (userName in memberships)
   {
      ...
      if (userName.match("^GROUP_"))
      {
         if (authorityType == null || authorityType == "GROUP")
         {
            ...
            zones['_' + userName] = "null";//No zones are populated for groups
         }
      }
      else
      {
         if (authorityType == null || authorityType == "USER")
         {
            ...
            roles['_' + userName] = membershipRole;
            var zone;
            if (resetPasswordService.isAdminUser(userName)) {
            	zone = "admin";
            } else if (resetPasswordService.isUserInternalUser(userName)) {
            	zone = "internal";
            } else {
            	zone = "external";
            }
            zones['_' + userName] = zone;
         }
      }
   }
