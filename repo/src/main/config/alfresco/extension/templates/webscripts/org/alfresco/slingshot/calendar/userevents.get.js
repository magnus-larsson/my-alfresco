...
// @overridden projects/remote-api/config/alfresco/templates/webscripts/org/alfresco/slingshot/calendar/userevents.get.js
...
model.events = getUserEvents(username, siteId, range, size);
 ...
 */
function getUserEvents(user, siteId, range, size)
{
   if (!user)
   {
      return [];
   ...
   if (siteId == null)
   {
      sites = siteService.listUserSites(user, size);
   }
   else
   {
   ...
   if (paths.length > 0)
   {
      ...
      if (range.fromdate)
      {
         ftsQuery = '+ia:fromDate:["' + range.fromdate + '" TO MAX] AND +(|';
      }
      else
      {
         ftsQuery = "+TYPE:\"{http\://www.alfresco.org/model/calendar}calendarEvent\" AND +(|";
      }
      ftsQuery += paths.join(" |") + ")";
      ...
      sortColumns.push(
      {
         column: "@" + utils.longQName("ia:fromDate"),
         ascending: true
      });
      var queryDef = {
         query: ftsQuery,
