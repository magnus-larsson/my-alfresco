...
// @overridden projects/remote-api/config/alfresco/templates/webscripts/org/alfresco/slingshot/documentlibrary/evaluator.lib.js

var Evaluator =
{
   _site_regexp: new RegExp('/app:company_home/st:sites/cm:([^/]+)/.*'),
   ...
   isSiteManager: function Evaluator_isSiteManager(node) {
      if (node.qnamePath) {
         //suck out site name from display path
         var result = Evaluator._site_regexp.exec(node.qnamePath);

         if (result != null && result.length > 1) {
            var site = Common.getSite(result[1]); //first in list is always entire matched string, second is first group etc..

            if (site != null) {
               var membersRole = site.getMembersRole(person.properties.userName) + "";
               return membersRole === 'SiteManager';
            }
         }
      }
   ...
   isSiteCollaborator: function Evaluator_isSiteCollaborator(node) {
      if (node.qnamePath) {
         // suck out site name from display path
         var result = Evaluator._site_regexp.exec(node.qnamePath);
         ...
         if (result != null && result.length > 1) {
            ...
            if (site != null) {
               var membersRole = site.getMembersRole(person.properties.userName) + "";
               return membersRole === 'SiteCollaborator';
            }
         }
      }
   ...
   run: function Evaluator_run(node, isParent)
   {
       ...
       * COMMON TO ALL
       */
      var pub = Published.isPublished(node);
      var siteManager = Evaluator.isSiteManager(node);
      var siteCollaborator = Evaluator.isSiteCollaborator(node);
      var documentPublisher = siteManager || (Published.publishAllowed(node) && siteCollaborator);
      ...
      permissions =
      {
         ...
         "unpublished": pub.published == false,
         "published-before": pub.hasbeen == true,
         "will-be-published": pub.future == true,
         "older-version-published": pub.publishedold,
         "site-manager": siteManager,
         "site-collaborator": siteCollaborator,
      ...
      switch (nodeType)
      {
         ...
         case "folder":
            ...
            if (node.hasAspect("vgr:auto-publish")) {
               if (node.properties['vgr:auto_publish_major_version']) {
                  status["auto-publish-major-version"] = true;
               }

               if (node.properties['vgr:auto_publish_all_versions']) {
                  status["auto-publish-all-versions"] = true;
               }
            }
