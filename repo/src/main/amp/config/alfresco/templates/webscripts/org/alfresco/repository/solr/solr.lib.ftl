<#-- @overridden projects/remote-api/config/alfresco/templates/webscripts/org/alfresco/repository/solr/solr.lib.ftl -->
<#macro json_string string>${string?js_string?replace("\\'", "\'")?replace("\\>", ">")}</#macro>
<#-- js_string does not convert unicode characters < u0020 in a good way. It produces invalid json code. Fix this!  -->
<#macro json_string_unicode string>${string?js_string?replace("\\'", "\'")?replace("\\>", ">")?replace("\\x", "\\u00")}</#macro>
...
<#macro nodeMetaDataJSON nodeMetaData filter>
      {
         ...
         , "parentAssocs": [
           ...
           "<@json_string_unicode "${pa}"/>"<#if pa_has_next>,</#if>
           </#list>
         ]
         ...
         , "childAssocs": [
           ...
           "<@json_string_unicode "${ca}"/>"<#if ca_has_next>,</#if>
           </#list>
         ]
         </#if>
         </#if>
         ...
         , "owner": "<@json_string_unicode "${nodeMetaData.owner}"/>"
         </#if>
         </#if>
      }
</#macro>
